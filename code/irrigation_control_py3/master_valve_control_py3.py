# This class controls the Main Valve and monitors for excessive flow
# The main valve is turned of for two reasons
# 1. Commands from irrigation programs
# 2. Commands from web where the valve will stay open a for a tbd time
# 3. Problem comes during cleaning where main control must be turned off during part of test
import time 
class Main_Valve(object):
   ...
   def __init__( self, cluster_id, cf,cluster_control, irrigation_io, handlers,current_operation,failure_report,irrigation_excessive_flow_limits,irrigation_hash_control):
       ...
       # Main valve off
       # Main valve control disabled
       # Main valve time condition
       ...
       self.irrigation_io.turn_off_main_valves_direct() 
   ...
   def construct_chains( self, cf ):
       ...
       cf.insert.check_event_no_init("IRI_OPEN_MASTER_VALVE", self.turn_on_main_valves )
       ...
       cf.define_chain("MV_ON_MONITOR",False) # make sure that there is no flow if main valve is off
       # take care of plc controller and main valves
       cf.insert.log("mv on handler")
       
       cf.insert.one_step(self.turn_on_main_valves)
       ...
       cf.insert.verify_function_reset( reset_event=None,reset_event_data=None, 
                                        function = self.monitor_main_valve_open )
       cf.insert.one_step(self.turn_off_main_valves)
       cf.insert.send_event(event="RELEASE_IRRIGATION_CONTROL" )
       cf.insert.one_step( self.report_excessive_flow ) #main valve has been detected to be off
       ...
       cf.insert.check_event_no_init("IRI_CLOSE_MASTER_VALVE", self.turn_off_main_valves)
       cf.insert.check_event_no_init("IRI_MASTER_VALVE_SUSPEND", self.change_to_offline_state ) 
       #cf.insert.check_event_no_init("IRI_MASTER_VALVE_RESUME", self.offline_exit ) #do nothing for this event
       #cf.insert.check_event_no_init("IRI_EXTERNAL_CLOSE", self.cancel_timed_state )
       cf.insert.check_event_no_init("IRI_EXTERNAL_TIMED_OPEN",self.change_to_timed_state)       
       ...
       cf.define_chain("MV_OFF_MONITOR",False) # make sure that there is no flow if main valve is off
       # take care of plc controller and main valves
       ...
       cf.insert.verify_function_reset( reset_event=None,reset_event_data=None, 
                                        function = self.monitor_main_valve_close )
       cf.insert.one_step(self.irrigation_io.turn_off_pump) # currently system doesnot have this capability
       cf.insert.one_step( self.report_main_valve_failure ) #main valve has been detected to be off
       cf.insert.terminate()
       ...
       cf.insert.one_step( self.turn_on_main_valves_timed )
       ...
       cf.insert.one_step(self.turn_off_main_valves)
       cf.insert.send_event(event="RELEASE_IRRIGATION_CONTROL" )
       cf.insert.one_step( self.report_excessive_flow_timed ) #main valve has been detected to be off
       cf.insert.one_step(self.change_to_off_state)
   ...
   def change_to_off_state(self,*parms):
      #print("change_to_off_state")
      self.main_valve_close_count = 0
   ...
   def offline_state_exit(self,*parms):
      ...
      if self.current_state == "MV_TIME_CYCLE":
         ...
         self.change_to_off_state()
      elif self.current_state == "MV_ON":
         self.change_to_on_state()
      else:
          raise ValueError("Improper state  "+self.current_state)
   ...
   def exit_timed_state(self,*parms):
       #print("exit time state") 
       self.ref_time = time.time()
       if self.main_valve_state == True:
         self.change_to_on_state()
       else:
         self.change_to_off_state()
 
   def set_timed_close_flag(self,*params):
       self.main_valve_state = False
 
   def turn_on_main_valves(self,*parms):
       self.main_valve_state = True
       self.irrigation_io.turn_on_main_valves_direct()


 
   def turn_off_main_valves(self,*parms):
       self.main_valve_state = False
       self.irrigation_io.turn_off_main_valves_direct()
       
   def turn_on_main_valves_timed(self,*parms):
   ...
   #
   # 
   def monitor_main_valve_open(self,cf_handle, chainObj, parameters, event):
       ...
       if event["name"] == "MINUTE_TICK":
           ...
           self.main_flow = self.handlers["MQTT_SENSOR_STATUS"].hget("MAIN_FLOW_METER")
           #print("main flow",self.main_flow)          
           if self.main_flow > self.irrigation_excessive_flow_limits["EXCESSIVE_FLOW_VALUE"]:
   ...
   def monitor_main_valve_close(self,cf_handle, chainObj, parameters, event):
       if event["name"] == "INIT":
          self.monitor_excessive_flow_count = 0
   ...
   def report_main_valve_failure(self,*parms):
     self.failure_report(self.current_operation,"MASTER_VALVE","OFF",{"flow_rate":self.main_flow}   )
     
     raise ValueError("Excessive Idle Main Valve Flow  "+str(self.main_flow))
    
   
   def report_excessive_flow(self,*parms):
     self.current_operation["flow_rate"] = self.main_flow
     
     self.handlers["IRRIGATION_PAST_ACTIONS"].push({"action":"EXCESSIVE_FLOW","details":self.current_operation,"level":"RED"})
     self.failure_report(self.current_operation,"MASTER_VALVE","ON",{"flow_rate":self.main_flow}   )
 
   def report_excessive_flow_timed(self,*parms):
       self.current_operation["flow_rate"] = self.main_flow
       self.handlers["IRRIGATION_PAST_ACTIONS"].push({"action":"EXCESSIVE_FLOW","details":self.current_operation,"level":"RED"})
       self.failure_report(self.current_operation,"MASTER_VALVE","TIMED_OPERATION",{"flow_rate":self.main_flow}   )   
